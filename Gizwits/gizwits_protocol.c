...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagwater_pump)
    {
        bitFieldBitLen += water_pump_LEN;
    }
    if(0x01 == aFlag->flagventilation_opening)
    {
        bitFieldBitLen += ventilation_opening_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwater_pump)
    {
        info->event[info->num] = EVENT_water_pump;
        info->num++;
        dataPoints->valuewater_pump = gizVarlenDecompressionValue(bitFieldOffset,water_pump_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += water_pump_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagventilation_opening)
    {
        info->event[info->num] = EVENT_ventilation_opening;
        info->num++;
        dataPoints->valueventilation_opening = gizVarlenDecompressionValue(bitFieldOffset,ventilation_opening_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += ventilation_opening_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuewater_pump != cur->valuewater_pump)
    {
        GIZWITS_LOG("valuewater_pump Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwater_pump = 1;
        ret = 1;
    }
    if(last->valueventilation_opening != cur->valueventilation_opening)
    {
        GIZWITS_LOG("valueventilation_opening Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagventilation_opening = 1;
        ret = 1;
    }
    ...
    if(last->valuesoil_temperature != cur->valuesoil_temperature)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesoil_temperature Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagsoil_temperature = 1;
            ret = 1;
        }
    }
    if(last->valuesoil_humidity != cur->valuesoil_humidity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesoil_humidity Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagsoil_humidity = 1;
            ret = 1;
        }
    }
    ...
    if(last->valueCO2 != cur->valueCO2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagCO2 = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagwater_pump)
    {
        gizVarlenCompressValue(bitFieldOffset,water_pump_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuewater_pump);
        bitFieldOffset += water_pump_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagventilation_opening)
    {
        gizVarlenCompressValue(bitFieldOffset,ventilation_opening_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueventilation_opening);
        bitFieldOffset += ventilation_opening_LEN;
    }
    ...
    {
        if(dataPoints->valueModel >= Model_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valueModel Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagsoil_temperature)
    {
        devStatusTmp.valuesoil_temperature = gizY2X(soil_temperature_RATIO,  soil_temperature_ADDITION, dataPoints->valuesoil_temperature); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuesoil_temperature,soil_temperature_LEN);
        byteFieldOffset += soil_temperature_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagsoil_humidity)
    {
        devStatusTmp.valuesoil_humidity = gizY2X(soil_humidity_RATIO,  soil_humidity_ADDITION, dataPoints->valuesoil_humidity); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuesoil_humidity,soil_humidity_LEN);
        byteFieldOffset += soil_humidity_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagCO2)
    {
        devStatusTmp.valueCO2 = exchangeBytes(gizY2X(CO2_RATIO,  CO2_ADDITION, dataPoints->valueCO2)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueCO2,CO2_LEN);
        byteFieldOffset += CO2_LEN;
    }
